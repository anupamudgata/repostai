...
create table if not exists public.connected_accounts (
  ...
  scope text,
  meta jsonb,
  -- Legacy columns used by Twitter OAuth callback
  provider text,
...
-- profiles: add photo tracking + updated_at + starter plan
...
-- subscriptions: add razorpay columns + starter plan + expanded statuses + unique user_id
...
-- Make stripe_subscription_id nullable (Razorpay subscriptions don't have one)
alter table public.subscriptions alter column stripe_subscription_id drop not null;
-- current_period_end can be null for Razorpay flows
...
-- Ensure unique user_id for upsert onConflict: "user_id"
do $$ begin
  if not exists (
...
-- brand_voices: add all authenticity + persona columns
...
-- Copy sample_text → samples for any existing rows
do $$
begin
  if exists (
...
-- repurpose_jobs: add outputs + status + relax input_content
...
-- Drop + re-add constraints for expanded values
...
-- scheduled_posts: add published_at + updated_at + expand status
alter table public.scheduled_posts
  add column if not exists published_at timestamptz,
  add column if not exists updated_at timestamptz default now();

-- Make connected_account_id and output_id nullable (photo posts don't use them)
...
-- repurpose_jobs (includes UPDATE policy)
...
-- repurpose_outputs (includes DELETE policy)
...
-- scheduled_posts (includes DELETE policy)
...
-- razorpay_orders (service-role only — no user-facing RLS policies needed,
-- but we enable RLS so it's locked down by default)
drop policy if exists "Service role full access razorpay_orders" on public.razorpay_orders;
...
-- Expected tables (13):
--   brand_voices, connected_accounts, created_posts, photo_caption_runs,
--   photo_uploads, post_engagement, profiles, razorpay_orders,
--   repurpose_jobs, repurpose_outputs, scheduled_posts, subscriptions, usage
-- ============================================
