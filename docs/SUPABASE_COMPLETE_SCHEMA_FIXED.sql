-- ============================================
-- RepostAI — Complete schema + migration (FIXED)
...
-- Drop old constraints BEFORE table creation (so CREATE IF NOT EXISTS doesn't conflict)
DO $$ BEGIN
...
-- NOTE: output_language includes ALL 11 languages from the start
...
-- NOTE: output_language includes ALL 11 languages
...
-- CRITICAL: Use ALL 11 languages (not just 5!)
alter table public.repurpose_jobs drop constraint if exists repurpose_jobs_output_language_check;
...
-- profiles (SELECT + UPDATE + INSERT — INSERT was missing before!)
...
-- razorpay_orders (service-role only)
drop policy if exists "Service role full access razorpay_orders" on public.razorpay_orders;

-- ============================================
-- 5. TRIGGER — auto-create profile on signup (with ON CONFLICT)
-- ============================================
...
-- ============================================
-- 6. RPC — ensure_profile_from_auth (JWT-based fallback)
-- ============================================
...
-- ============================================
-- 8. BACKFILL — create profiles for existing users who don't have one
-- ============================================
...
-- Expected: 13 tables
--
-- Verify profile backfill:
--   select count(*) from auth.users u left join profiles p on u.id = p.id where p.id is null;
-- Expected: 0 (all users have profiles now)
--
-- Verify RPC:
--   select routine_name from information_schema.routines where routine_name = 'ensure_profile_from_auth';
-- Expected: 1 row
-- ============================================
