...
export async function POST(request: NextRequest) {
  try {
    ...
    let videoId: string | null = url?.trim()
    ...
    if (transcript?.trim()) {
      ...
      if (parsed.length < 2) {
        return NextResponse.json(
          {
            error:
              "Could not parse enough timestamps. Use format [M:SS] or [H:MM:SS] followed by text, one per line.",
          },
          { status: 400 }
        );
      }
      ...
      return NextResponse.json(
        { error: "YouTube URL or transcript is required" },
        { status: 400 }
      );
      ...
      let segs: { text: string; start: number; dur: number }[];

      // FIX: try/catch properly nested inside else block
      try {
        ...
        if (
          ...
          msg.includes("No caption tracks") ||
          msg.includes("Transcript too short")
        ) {
          return NextResponse.json(
            {
              error:
                'No captions or transcript available for this video. Switch to "Paste transcript" and copy the transcript from YouTube (CC → Transcript), or try a different video with subtitles enabled.',
            },
            { status: 400 }
          );
        }
        ...
        if (msg.includes("Invalid YouTube URL")) {
          return NextResponse.json(
            { error: "Invalid YouTube URL. Use a link like youtube.com/watch?v=..." },
            { status: 400 }
          );
        }
        ...
        if (
          ...
          return NextResponse.json(
            {
              error:
                "Could not extract transcript from this video. Ensure the video has captions enabled and is publicly accessible.",
            },
            { status: 400 }
          );
        }
        ...
        if (msg.includes("Failed to fetch") || msg.includes("fetch")) {
          return NextResponse.json(
            {
              error:
                "Could not reach YouTube. The video may be private, age-restricted, or unavailable. Try a different video.",
            },
            { status: 400 }
          );
        }
        ...
        return NextResponse.json(
          {
            error:
              msg || "Could not process this video. Try a video with subtitles enabled.",
          },
          { status: 400 }
        );
      }
    ...
    // ── Ask AI to identify clip moments ──────────────────────────────────────
...
- Spread clips across the video. Do not overlap.
...
- suggestedLength: one of "15-60", "60-90", or "90-180".`,
        },
        {
          role:    "user",
    ...
    // FIX: explicit catch parameter to avoid SWC optional-catch-binding issues
    try {
